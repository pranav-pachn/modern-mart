...
export async function POST(
  ...
  try {
    ...
    // Authenticate user via NextAuth token. The frontend uses Auth.js v5 which uses a different cookie name than v4.
    const isSecure = request.url.startsWith("https://");
    const cookieName = isSecure ? "__Secure-authjs.session-token" : "authjs.session-token";
    
    // Fallback to standard v4 names just in case
    ...
    const existingReview = await db.collection<ReviewDocument>(REVIEWS_COLLECTION).findOne({
      ...
      userId: token.email, // Using email as user identifier since we have it reliably in JWT
    });
