...
export async function GET(request: NextRequest) {
  try {
    ...
    const [statusBreakdown, revenueByDay, ordersByDay, topProducts, recentActivity] = await Promise.all([
      col.aggregate([
    ...
    return NextResponse.json({ error: "Failed to fetch order analytics." }, { status: 500, headers: corsHeaders });
  }
}
