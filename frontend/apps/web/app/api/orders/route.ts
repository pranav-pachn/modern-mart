...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, userEmail, userName, phone, address, deliverySlot, subtotal, items, total, paymentMethod, notes } = parsed.data;

    if (subtotal < MINIMUM_ORDER_VALUE) {
      return jsonWithCors({ error: `Minimum order value is Rs. ${MINIMUM_ORDER_VALUE}.` }, 400);
    }
    ...
    for (const item of items) {
      const dbProduct = productsInDb.find((product) => product._id?.toString() === item.productId);
      if (!dbProduct) {
    ...
    return jsonWithCors({ orderId: result.insertedId.toString() }, 201);
  } catch (error: unknown) {
    ...
    return jsonWithCors({ error: "Unable to place order." }, 500);
  }
}
  ...
  try {
    ...
    if (searchParams.get("stats") === "1") {
      ...
      const [totalOrders, todayOrders, pendingOrders, revenueResult] = await Promise.all([
        ...
        col.aggregate([{ $group: { _id: null, total: { $sum: "$total" } } }]).toArray(),
      ]);
    ...
    return jsonWithCors(
      {
        ...
        details: process.env.NODE_ENV !== "production" ? String((error as Error).message) : undefined,
      },
      500
    );
  }
...
function isMongoValidatorError(error: unknown): error is { code: number; errInfo?: unknown } {
