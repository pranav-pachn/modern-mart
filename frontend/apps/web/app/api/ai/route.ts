...
export async function POST(req: Request) {
  try {
    ...
    const prompt = typeof (rawBody as { prompt?: unknown })?.prompt === "string" ? (rawBody as { prompt: string }).prompt : "";
    ...
    const hasValidKey = providers.some((provider) => !provider.key.includes("your_"));
    if (!hasValidKey) {
...
7. Output ONLY valid JSON. No markdown formatting, no backticks, no explanations.`;
    ...
    if (!success || !responseData) {
      if (providerErrors.length === providers.length) {
        return NextResponse.json({ error: "All AI providers are currently unavailable.", details: providerErrors }, { status: 503, headers: corsHeaders });
      }

      return NextResponse.json({ error: "AI service failed to generate response.", details: lastErrorDetails ?? providerErrors }, { status: 502, headers: corsHeaders });
    }
